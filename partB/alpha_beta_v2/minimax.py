...
from alpha_beta_v2.evaluation import get_difference, get_longest_component, get_score
...
def minimax(board, action, depth, alpha, beta, player):
    ...
    if player == 'blue': # why blue here???
        ...
        for a in board_copy.get_actions():
            v = minimax(board_copy, a, depth - 1, alpha, beta, _SWAP_PLAYER[player])
            if v >= beta:
                break
            alpha = max(alpha, v)
